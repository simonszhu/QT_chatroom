    ...
    , ui(new Ui::ChatRoom)
{
    ...
    setMinimumSize(400, 300);//设置窗口最小尺寸为800*600
    resize(400, 300);
    ...
    UserNameLineEdit->setPlaceholderText("您的用户名");
...
//    ChatRoomTextEdit->setDisabled(true);
    ...
    FlushOnline->setGeometry(this->width() * 0.7, 10, this->width() * 0.3, 24);
...
//    设置事件与槽
//    readyread查看是否有数据到达，disconnected连接断开
...
ChatRoom::~ChatRoom()
{
    if(ChatRoomTextEdit != nullptr)
        delete ChatRoomTextEdit;
    if(InputTextEdit != nullptr)
        delete InputTextEdit;
    if(OnlineCountLabel != nullptr)
        delete OnlineCountLabel;
    if(UserNameLineEdit != nullptr)
        delete UserNameLineEdit;
    if(AddChatRoom != nullptr)
        delete AddChatRoom;
    if(SendMessage != nullptr)
        delete SendMessage;
    if(FlushOnline != nullptr)
        delete FlushOnline;
    if(chatSocket != nullptr)
        delete chatSocket;
    if(onlineSocket != nullptr)
        delete onlineSocket;
    delete ui;
}
...
//    }
    ...
    if(Username != user_name){
        QMessageBox(QMessageBox::Warning, "警告", "您输入的用户名与登录用户名不相符...", QMessageBox::Yes).exec();
        return;
    }
...
//        连接服务器connectToHost，默认使用IPV6
        chatSocket->connectToHost(chatAddress, chatPort, QAbstractSocket::ReadWrite, QAbstractSocket::IPv4Protocol);
//        等待服务器端连接
...
//        默认使用IPV6
        onlineSocket->connectToHost(onlineAddress, onlinePort, QAbstractSocket::ReadWrite, QAbstractSocket::IPv4Protocol);
...
{
//    readAll函数是QIODevice中的读写函数，是QTCPsocket的父类QAbstractSocket的父类
